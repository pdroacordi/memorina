...
- Nothing in the world grants `PlayerItem.MEMORINA` or calls `SaveSystem.learn_song()` yet; there are no pickups and no benches, and `save_game()` is still never called. The debug action `debug_learn_song` (F9, debug builds only) grants the instrument and teaches the next unknown song with the full lesson flow — it stands in for the guardian fights and goes away when they exist.
